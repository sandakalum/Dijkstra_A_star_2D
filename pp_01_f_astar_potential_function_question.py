    ...
    while front:
        ...
        for dx, dy, deltacost in movements:
            ...
            # value is 255 (check for '==255', not for '> 0').
                if not(visited[new_pos]==1 or obstacles[new_pos] == 255):# If visited is 0 and obstacles is not 255 (both at new_pos), then:
                    heappush(front, (cost + deltacost + distance(new_pos,goal) + obstacles[new_pos] / 64, cost + deltacost + obstacles[new_pos] / 64, new_pos, pos))# append the tuple (cost + deltacost, new_pos) to the front.
